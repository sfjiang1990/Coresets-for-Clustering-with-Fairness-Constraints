...
from util.clusteringutil import (clean_data, read_data, scale_data,
                                 ...
                                 vanilla_clustering_weighted, write_fairness_trial)
from util.configutil import read_list
from util.weightutil import weighted_size_by_idx
...
def fair_clustering(dataset, config_file, data_dir, num_clusters, deltas, max_points, violating, violation):
    ...
    for var, bucket_dict in attributes.items():
        ...
        representation[var] = {k : ((weighted_size_by_idx(weight, bucket_dict[k])) / weighted_size(weight)) for k in bucket_dict.keys()}
    ...
    if not violating:
        t1 = time.monotonic()
        initial_score, pred, cluster_centers = vanilla_clustering_weighted(df, weight, num_clusters, clustering_method)
        t2 = time.monotonic()
        cluster_time = t2 - t1
    ...
    for delta in deltas:
        ...
        if not violating:
            t1 = time.monotonic()
            res = fair_partial_assignment(df, weight, cluster_centers, fp_alpha, fp_beta, fp_color_flag, clustering_method)
            t2 = time.monotonic()
            lp_time = t2 - t1
