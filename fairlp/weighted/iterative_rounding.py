...
import time
from util.weightutil import weighted_size


def iterative_rounding_lp(df, weight, centers, distance, color_flag, res):
    ...
    objects_returned = preprocess_lp_solution(lp_assignment, weight, num_points, num_centers)
    integral_assignment, frac_lp_assgn = objects_returned

    # The following part is for debugging purposes
    #obj_ret = find_lp_cost(lp_assignment,distance,num_centers,num_points)
    ...
    while weighted_size(weight) != 0:
        res = mbdmb_lp_solver(distance, num_centers, num_points, weight, frac_lp_assgn, color_flag)
        new_lp_assignment = res["assignment"]
        objects_returned = update_assignment(num_centers, num_points, weight, frac_lp_assgn, new_lp_assignment, integral_assignment)
        integral_assignment, frac_lp_assgn = objects_returned

    final_cost =  find_integral_cost(integral_assignment, distance, num_points, num_centers)
    #res["assignment"] = reformat_assignment (integral_assignment, num_centers)
    res["assignment"] = integral_assignment
...
def preprocess_lp_solution(lp_assignment, weight, num_points, num_centers):
    ...
    integral_assignment = np.full(shape=(num_points, num_centers),fill_value=0)
    #frac_list = []
    frac_lp_assgn = []

    for i in range(num_points):
        for j in range(num_centers):
            int_part = math.floor(assignment[i][j])
            integral_assignment[i][j] += int_part
            assignment[i][j] -= int_part
            weight[i] -= int_part
    ...
    return integral_assignment, frac_lp_assgn


def mbdmb_lp_solver(distance, num_centers, num_points, weight, frac_lp_assgn, color_flag):
    ...
    t1 = time.monotonic()
    objects_returned = prepare_to_add_variables(distance, num_centers, num_points, frac_lp_assgn)
    ...
    t1 = time.monotonic()
    objects_returned = prepare_to_add_constraints(num_centers, num_points, weight, frac_lp_assgn, color_flag)
...
def prepare_to_add_variables(distance, num_centers, num_points, frac_lp_assgn):
    ...
    return objective, lower_bounds, variable_names


def prepare_to_add_constraints(num_centers, num_points, weight, frac_lp_assgn, color_flag):

    # The following steps constructs the assignment constraint. Each corresponding row in A
    # has #centers many non-zero entries.
    constraints_row, rhs = constraint_sums_to_one(num_centers, num_points, weight, frac_lp_assgn)
    ...
    obj_ret = constraint_fairness(num_centers, num_points, frac_lp_assgn, color_flag)
...
def constraint_sums_to_one(num_centers, num_points, weight, frac_lp_assgn):
...
def constraint_fairness(num_centers, num_points, frac_lp_assgn, color_flag):
        ...
        for j in range(num_points):
            for i in range(num_centers):
                ...
                if frac_lp_assgn[j][i] != 0.0:
                    ...
                    T_f_l[var][color][i] += assignment[j][i]
...
def update_assignment(num_centers, num_points, weight, frac_lp_assgn, new_lp_assignment, integral_assignment):
    ...
    for i in range(num_points):
        for j in range(num_centers):
            ...
            assignment[i][j] -= int_part
            weight[i] -= int_part
        new_frac_lp_assgn.append(assignment[i].tolist())
    #for index,j in enumerate(frac_list):
    ...
    return integral_assignment, new_frac_lp_assgn


def find_integral_cost (integral_assignment, distance, num_points, num_centers):

    cost = 0
    for i in range(num_points):
        for j in range(num_centers):
            cost += integral_assignment[i][j] * distance[i * num_centers + j]
