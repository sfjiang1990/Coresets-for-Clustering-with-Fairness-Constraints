...
class Clustering:
    ...
    @staticmethod
    def closest(point, C):
        return np.argmin([np.linalg.norm(point - C[i]) for i in range(len(C))])

    @staticmethod
    def dist(point, C):
        #l = [np.linalg.norm(point - plist[i]) for i in C]
        return min(np.linalg.norm(point - q) for q in C) 
        #return min(l)
    
    @staticmethod
    def evaluate(plist, weight, C):
        #res = np.sum(Clustering.dist(plist, plist[i], C, DIST) * weight[i] for i in range(len(plist)))
        return sum(Clustering.dist(plist[i], C) * weight[i] for i in range(len(plist)))

    @staticmethod
    def dist2(point, C):
        #l = [np.linalg.norm(point - plist[i]) for i in C]
        return math.pow(Clustering.dist(point, C), 2)
        #return min(l)
    
    @staticmethod
    def evaluate2(plist, weight, C):
        #res = np.sum(Clustering.dist(plist, plist[i], C, DIST) * weight[i] for i in range(len(plist)))
        return sum(Clustering.dist2(plist[i], C) * weight[i] for i in range(len(plist)))

    @staticmethod
    def get_center(plist, cluster, weight):
        tot_w = sum(weight)
        return np.sum((plist[i] * weight[i] for i in cluster),axis=0) / tot_w

    @staticmethod
    def kmeans(df, weight, k):
        opt = 1e50
        C = []
        pred = []
        for i in range(5):
            v, p, c = Clustering._kmeans(df, weight, k)
            if v < opt:
                opt = v
                C = c
                pred = p
    ...
    def _kmeans(df, weight, k):
        ...
        #C = [plist[i] for i in range(k)]
        rand = random.sample(range(n), k)
        C = [plist[i] for i in rand]
        current = Clustering.evaluate2(plist, weight, C)
        while True:
            clusters = {}
            for i in range(k):
                clusters[i] = []
            for i in range(n):
                id = Clustering.closest(plist[i], C)
            ...
            for i in range(k):
                if len(clusters[i]) != 0:
                    non_zero = i
                    break
            C = [Clustering.get_center(plist, clusters[i], weight) for i in range(k)]
            for i in range(k):
                if len(clusters[i]) == 0:
                    C[i] = C[non_zero]
            new_cost = Clustering.evaluate2(plist, weight, C)
            print((current-new_cost) / current)
            if ((current - new_cost) / current < 0.1) and (new_cost <= current):
                pred = [ Clustering.closest(plist[i], C) for i in range(n)]
                return np.sqrt(new_cost), pred, C
            current = new_cost


    @staticmethod
    def kmedian(df, weight, k):
        plist = []
        for row in df.iterrows():
            i_point, data = row
            point = data.tolist()
        ...
        #DIST = ([ [ np.linalg.norm(p - q) for q in plist] for p in plist ])
        ...
        while True:
            ...
            if flag == False:
                ...
                for p in sol:
                    C.append(p)
                obj = Clustering.evaluate(plist, weight, C, DIST)
                pred = [ Clustering.closest(i, C, DIST) for i in range(n)]
                return obj, pred, [df.iloc[c].data.tolist() for c in C]
            sol.remove(delete)
            sol.add(insert)
...
def clean_data(df, config, dataset):
    ...
    return df, variable_columns, [p for p in df["multiplicity"]]
...
def vanilla_clustering_weighted(df, weight, num_clusters, clustering_method):
    if clustering_method == 'kmedian':
        return Clustering.kmedian(df, weight, num_clusters)
    elif clustering_method == 'kmeans':
        kmeans = KMeans(num_clusters)
        kmeans.fit(X=df, sample_weight=weight)
        initial_score = np.sqrt(-kmeans.score(df))
        pred = kmeans.predict(df)
        cluster_centers = kmeans.cluster_centers_
        return initial_score, pred, sort_centers(cluster_centers)
        #return Clustering.kmeans(df, weight, num_clusters)
    else:
