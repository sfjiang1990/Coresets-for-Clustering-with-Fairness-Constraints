...
def fair_partial_assignment(df, weight, centers, alpha, beta, color_flag, clustering_method):
    ...
    if clustering_method == "kmeans" or clustering_method == "kmedian":
        ...
        problem, objective = fair_partial_assignment_lp_solver(df, weight, centers, color_flag, alpha, beta, cost_fun_string)
        ...
        final_res = iterative_rounding_lp(df, weight, centers, objective, color_flag, res)
...
def fair_partial_assignment_lp_solver(df, weight, centers, color_flag, alpha, beta, cost_fun_string):
    ...
    print("HERE???")
    t1 = time.monotonic()
    objective, lower_bounds, variable_names = prepare_to_add_variables(df, centers, cost_fun_string)
    ...
    t1 = time.monotonic()
    objects_returned = prepare_to_add_constraints(df, weight, centers, color_flag, beta, alpha)
...
def prepare_to_add_variables(df, centers, cost_fun_string):
    ...
    print("TOTAL", num_points, num_centers)
...
def prepare_to_add_constraints(df, weight, centers, color_flag, beta, alpha):
    ...
    constraints_row, rhs = constraint_sums_to_one(num_points, num_centers, weight)
...
def constraint_sums_to_one(num_points, num_centers, weight):

    constraints = [[["x_{}_{}".format(j, i) for i in range(num_centers)], [1] * num_centers] for j in range(num_points)]
    rhs = [float(weight[i]) for i in range(num_points)]
    #rhs = [1] * num_points
